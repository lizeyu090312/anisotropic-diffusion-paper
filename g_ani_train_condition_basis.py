...
from common_utils import GFn, flow_matching_energy_debug_basis_condition_v3, ANI_absM_Precond_Flow_Net_basis_v3
...
def build_model_from_gonly(dataset, V_dim, res, device, out, outdir: pathlib.Path,
    ...
    return raw, ema, g_fns, h_fns, V0, V_by_label, label2basis
...
def train(opt):
    ...
    # model
    model, ema, g_fns, h_fns, V0, V_by_label, label2basis = build_model_from_gonly(
    ...
    for step in range(steps_total):
        ...
        for x_chunk, y_chunk in zip(x_chunks, y_chunks):
            if opt.dataset == "imagenet":
                ...
                with autocast_ctx():
                    loss = flow_matching_energy_debug_basis_condition_v3(
                        ddp_model, x_chunk,
                        labels_id=basis_chunk,
                        labels=y_chunk,
                        g_fn=g_fns, h_fn=h_fns,
                        dataset=opt.dataset,
                        T=T,
                        V=V_by_label,
                    )
            ...
            used_labels.update(basis_chunk.unique().tolist())
        ...
        if (step + 1) % 100 == 0:
            ...
            if is_main():
                if opt.keep_all_ckpt:
                    ckpt_path = outdir / f"finetuned-g-ani-rampup-{lr_rampup_kimg}-ema-{ema_halflife_kimg}-basis-v3-newpca-condition-ddp-ckpt-{(step+1):05d}.pkl"
                else:
                    ckpt_path = outdir / f"finetuned-g-ani-rampup-{lr_rampup_kimg}-ema-{ema_halflife_kimg}-basis-v3-newpca-condition-ddp.pkl"
    ...
    barrier(device_ids=[local_rank])
    if is_main():
        final_path = outdir / f"finetuned-g-ani-rampup-{lr_rampup_kimg}-ema-{ema_halflife_kimg}-basis-v3-newpca-condition-ddp.pkl"
