...
def load_model(dataset, model_tag, ckpt_root, device, args=None):
    ...
    if "ani-edm" in model_tag:
        ...
        print("Detected anisotropic RANGE model → rebuilding selectors")
        ...
        print(f"Rebuilt g/h selectors with interval [{t_lo:.2f}, {t_hi:.2f}]")

    else:
        if "g" in ckpt:
            g_fn = ckpt["g"].to(device)
        elif "g_fn" in ckpt:
            g_fn = ckpt["g_fn"].to(device)
        else:
        ...
        print(f"Loaded schedules: g={'yes'} h={'yes' if ('h' in ckpt) else 'no(h=g)'} ; ema={'yes'}")
    ...
    return edmnet, g_fn, h_fn, V_by_label, V_global, label2basis
