...
def load_model(dataset, model_tag, ckpt_root, device, args):
    ...
    if "g_fns" in ckpt:
        ...
        # move to device + eval
        g_fns = {str(k): v.to(device).eval() for k, v in g_fns.items()}
        h_fns = {str(k): v.to(device).eval() for k, v in h_fns.items()}

        # build per-class f_fns
        f_fns = {}
        for k in g_fns.keys():
            f_fns[k] = _make_f_fn(g_fns[k], h_fns[k], device)
    ...
    return flow_net, res, g_fns, h_fns, f_fns, V_by_label, V_global, label2basis
...
def sample_images(dataset="imagenet", model_tag="g-ani", ckpt_root="finetune_initial",
    ...
    flow_net, res, g_obj, h_obj, f_obj, V_by_label, V_global, label2basis = load_model(
