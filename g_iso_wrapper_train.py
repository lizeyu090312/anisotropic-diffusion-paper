...
def main():
    ...
    log_file = outdir / "log_train_iso_wrapper.txt"
    ...
    g_iso_path = f"finetune/{opt.dataset}/finetuned-g-iso.pkl"
    g_disc_path = f"finetune/{opt.dataset}/finetuned-g-iso-discretize.pkl"

    print(f"Loading g-iso checkpoint: {g_iso_path}")
    ...
    while seen < total_nimg:
        ...
        if batches_done % 50 == 0:
            if opt.keep_all_ckpt:
                ckpt_path = outdir / f"finetuned-g-iso-wrapper-ckpt-{batches_done:05d}.pkl"
            else:
                ckpt_path = outdir / "finetuned-g-iso-wrapper.pkl"
            with open(ckpt_path, "wb") as f:
                pickle.dump({"ema": ema.cpu(), "g": g_fn.cpu(), "h": h_fn.cpu()}, f)
            print("Saved", ckpt_path)
            ema.to(device); g_fn.to(device); h_fn.to(device)

    final_path = outdir / "finetuned-g-iso-wrapper.pkl"
